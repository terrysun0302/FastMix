...
round(table(Data_1$X[,"Res"]),3)
round(table(Data_0$X[,"Res"]),3)
...
DiscrScores = score_func(mod1, Data_0, Data_1, Response_interaction_index = c(9,10,11), Response_idx=5)
...
```{r score2-summary, include=TRUE, echo=TRUE, results='asis', fig.width=10, fig.height=5}
...
hist(DiscrScores$single_sparse_score[1:n2/2], xlim = xl, main = "Distribution of the sparse score", xlab = "Sparse score", col=rgb(0.95,0.95,0.95,0.5))
...
These two scores, as well as their multivariate counterparts (entries `multi_score` and `multi_sparse_score` in object `DiscrScores`), can be used in linear or nonlinear machine learning algorithms to build the actual classifiers for the binary outcome.
