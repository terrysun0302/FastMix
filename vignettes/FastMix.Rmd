...
### this step cannot be wrap intot the score function because we need to check the Data2 to find the index
...
Note that we must let the scoring function know that which variable in the testing data is the binary outcome to be classified (function `DataPrep_test()` automatically renames this variable as `Res` in the processed data), and which variables are the interactions between `Res` and cell proportions.

By checking the first few rows of the processed data, we find that the $5$th column is the binary response variable, and the $9, 10, 11$th columns are the interaction terms between the response and cell proportions.  This information will be used in `score_func()` to produce several discriminant scores.

Finally, we show some summary plots and tables for the classification results. 


```{r score2, include=TRUE, echo=TRUE, results='asis', fig.width=12, fig.height=6}
#================================================================================#
#=============== step 3: clacluate the scores for classification ================#
#================================================================================#

# we can overview the data to find some useful index for the score calculation
# For the csSAM data, for example, we find the the response interaction term is at the 5,6,7th column (excluding ID column) of the data
# and response is at the 4th column of the data
...
par(mfrow=c(1,2))
xl <- c(min(predicted_score$single_score)-2, max(predicted_score$single_score)+2)
hist(predicted_score$single_score[1:n2/2], xlim = xl, main = "Distribution of the overall score", xlab = "Overall score", col=rgb(0.95,0.95,0.95,0.5))
hist(predicted_score$single_score[(n2/2+1):n2], col=rgb(0.05,0.05,0.05,0.5), add=TRUE)
legend("topright", c("Res = 0", "Res = 1"), col = c(rgb(0.95,0.95,0.95,0.5), rgb(0.05,0.05,0.05,0.5)), lwd = 10, cex = 0.5)

xl <- c(min(predicted_score$single_sparse_score)-2, max(predicted_score$single_sparse_score)+2)
hist(predicted_score$single_sparse_score[1:n2/2], xlim = xl, main = "Distribution of the sparse score", xlab = "Sparse score", col=rgb(0.95,0.95,0.95,0.5))
hist(predicted_score$single_sparse_score[(n2/2+1):n2], col=rgb(0.05,0.05,0.05,0.5), add=TRUE)
legend("topright", c("Res = 0", "Res = 1"), col = c(rgb(0.95,0.95,0.95,0.5), rgb(0.05,0.05,0.05,0.5)), lwd = 10, cex = 0.5)

## create a summary table
single_score = c(mean(predicted_score$single_score[1:n2/2]), sd(predicted_score$single_score[1:n2/2]),
                 mean(predicted_score$single_score[(n2/2+1):n2]), sd(predicted_score$single_score[(n2/2+1):n2]))
single_sparse_score = c(mean(predicted_score$single_sparse_score[1:n2/2]), sd(predicted_score$single_sparse_score[1:n2/2]),
                 mean(predicted_score$single_sparse_score[(n2/2+1):n2]), sd(predicted_score$single_sparse_score[(n2/2+1):n2]))

summy_stat = rbind(single_score, single_sparse_score)

### signal to noise ratio
summy_stat = cbind(summy_stat, (summy_stat[,3]- summy_stat[,1])/(summy_stat[,2]+summy_stat[,4])/2/sqrt(4/n2))
colnames(summy_stat) = c("mean", "sd", "mean", "sd", "SNR")

kable(round(summy_stat, 3))
```

From the plots and tables, we see that both the one-dimensional overall and sparse scores have discriminant powers to separate the two gender groups, and the sparse score seems to have better signal-to-noise ration (SNR) than the overall score.

These two scores, as well as their multivariate counterparts, can be used in linear or nonlinear machine learning algorithms to build the actual classifiers for the binary outcome.
