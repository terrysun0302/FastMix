...
1. All subjects are independent, but are not identically distributed. The variance of the $j$th sample is considered as $1/\mathrm{TNMR}_{j}$, where $\mathrm{TNMR}_{j}$ is generated from a uniform distribution $U(1, 10)$ (reads in millions).
...
TNMR <- runif(50, min=1, max=10)

## We assume that the variance is *inversely* proportional to TNMR for
## normalized expressions
cov_matrix <- diag(1/TNMR)
### we generate a weighted data
dat = data_gen(m, n, seed = 10, outlier = T, cor = 0.5, balance = F, weight = cov_matrix)
